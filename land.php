      ...
      <h3>enter book title to borrow</h3>
        <form class="card card-sm" action="" method="GET">
                    <div class="card-body row no-gutters align-items-center">
                        <!--end of col-->
                        <div class="col">
                        <input type="text" id="myInput" name="query" onkeyup="myFunction()" placeholder="Search for book..">
                        </div>
                        <!--end of col-->
                        <div class="col-auto" style="padding-left: 45%;">
                            <button class="btn btn-lg btn-success" type="submit" value="Search"><i class="fas fa-search" style="font-size: 20px; padding-right:2px;color:white;"></i></button>
                        </div>
                        <!--end of col-->
                    </div>
        </form>
									...
									if(strlen($query) >= $min_length){ // if query length is more or equal minimum length then
										...
										if($raw_results->num_rows > 0){ // if one or more rows are returned do following
											...
											// $results = mysql_fetch_array($raw_results) puts data from database into array, while it's valid it does the loop
											      // $field1name = $row["id"];
														$field2name = $row["book_isbn"];
														$field3name = $row["book_title"];
														$field4name = $row["book_author"];
                                          $field5name = $row["book_descr"]; 
                                          $field6name = $row["book_price"];
                                          ...
                                          <td>'.$field6name.'</td> 
                                            ...
                                            $sql = "UPDATE users SET requests='$field3name' WHERE username='$logedInUsername'";
